...
class UnscentKalman(KalmanFilter):
    ...
    def __init__(self, f_func, h_func, f_args, h_args, alpha=10**(-3), beta=2, kappa=0, d=2, sigma_a=100.0, image_size = (4094, 2046)):
    ...
    def update(self, delta_t, z, R, state, state_cov, pred_state, pred_cov):
        ...
        print(np.max(self.pred_state))
        print(np.max(self.pred_cov))
        self.icorrect.define_params(Xs)
        self.state, self.state_cov,  self.kalman_gain  = self.correct(z, R, self.pred_state, self.pred_cov, state, state_cov, delta_t)
