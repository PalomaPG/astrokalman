...
class RunData(object):
    ...
    def __init__(self,year = '15',only_HiTS_SN=True,test_SN=92,filter_type='kalman',n_params=0,results_dir='results'):
        self.year = year
...
class FITSHandler(object):
    ...
    def get_data_names(self):
        ...
        if glob('/home/phuente/MCKF'): # At Leftraru
            ...
            self.data_names['base'] = sorted(glob(base_dir + 'Blind' + self.year + 'A_' + self.field + '/*/' + self.ccd + '/Blind*_image.fits*'))[2]
            self.data_names['base_crblaster'] = sorted(glob(base_dir + 'Blind' + self.year + 'A_' + self.field + '/*/' + self.ccd + '/Blind*image_crblaster.fits*'))[2]
            self.data_names['science'] = sorted(glob(base_dir + 'Blind' + self.year + 'A_' + self.field + '/*/' + self.ccd + '/Blind*image_crblaster_grid02_lanczos2.fits'))
            ...
            for science_filename in self.data_names['science']:
                ...
                self.data_names['diff'] += [np.sort(glob(base_dir + 'Blind' + self.year + 'A_' + self.field + '/*/' + self.ccd + '/Diff*' + self.ccd + '_' + epoch + '*grid02*')).tolist()[0]]
                self.data_names['invVAR'] += [np.sort(glob(base_dir + 'Blind' + self.year + 'A_' + self.field + '/*/' + self.ccd + '/invVAR*' + self.ccd + '_' + epoch + '*grid02*')).tolist()[0]]
                self.data_names['psf'] += [np.sort(glob('/home/apps/astro/data/SHARED/Blind' + self.year + 'A_' + self.field + '/' + self.ccd + '/CALIBRATIONS/psf*' + self.ccd + '_' + epoch + '*grid02*')).tolist()[0]]
                self.data_names['aflux'] += [np.sort(glob('/home/apps/astro/data/SHARED/Blind' + self.year + 'A_' + self.field + '/' + self.ccd + '/CALIBRATIONS/match_*' + epoch +'-02.npy')).tolist()[0]]
            ...
            for science_filename in self.data_names['science']:
                ind = science_filename.find('_image_')
                epoch = science_filename[ind-2:ind]
                self.data_names['diff'] += [np.sort(glob(baseDir + 'diff/Diff*' + self.ccd + '_' + epoch + '*grid02*')).tolist()[0]]
                self.data_names['psf'] += [np.sort(glob(baseDir + 'psf/psf*' + self.ccd + '_' + epoch + '*grid02*')).tolist()[0]]
                self.data_names['invVAR'] += [np.sort(glob(baseDir + 'invvar/invVAR*' + self.ccd + '_' + epoch + '*grid02*')).tolist()[0]]
...
class KalmanFilter(object):
    ...
    def __init__(self,init_time=0.0,init_state=0.0,images_size=(4096,2048),initial_variance=100.0,sigma=1000.0,std_factor=100.0):
...
class MaximumCorrentropyKalmanFilter(KalmanFilter):
    ...
    def correct_with_measurements(self,z,R,epsilon=1e-6,max_iter=10,silverman_sigma=False,report_graphs=False):
        ...
        while True:
            ...
            stopped_pixels = np.linalg.norm(iter_state - prev_iter_state,axis=0) <= np.linalg.norm(prev_iter_state,axis=0)*epsilon
        ...
        self.state_cov[0,:] = np.power(1-self.kalman_gain[0,:],2)*self.pred_state_cov[0,:] + np.power(self.kalman_gain[0,:],2)*R
        self.state_cov[1,:] = (1-self.kalman_gain[0,:])*(self.pred_state_cov[1,:]-self.kalman_gain[1,:]*self.pred_state_cov[0,:]) + self.kalman_gain[0,:]*self.kalman_gain[1,:]*R
        self.state_cov[2,:] = np.power(self.kalman_gain[1,:],2)*self.pred_state_cov[0,:] -2.0*self.kalman_gain[1,:]*self.pred_state_cov[1,:] + self.pred_state_cov[2,:] + np.power(self.kalman_gain[1,:],2)*R
    ...
    def predict_for_Chen_test(self):
        ...
        self.pred_state_cov[0,:] = cos2*self.state_cov[0,:] - 2*cossin*self.state_cov[1,:] + sin2*self.state_cov[2,:] + Q[0]
        self.pred_state_cov[1,:] = cossin*(self.state_cov[0,:]-self.state_cov[2,:]) + (cos2-sin2)*self.state_cov[1,:] + Q[1]
        self.pred_state_cov[2,:] = sin2*self.state_cov[0,:] + 2*cossin*self.state_cov[1,:] + cos2*self.state_cov[2,:] + Q[2]
    ...
    def graph_diffs(self,C,figsize1=12,figsize2=8,row=0,outlier_percentage = 2.0):
        ...
        for d in range(3):
            ...
            plt.legend(loc=0)
        if row==1:
            plt.savefig('iterations/Obs-' + str(self.observation).zfill(2) + '_Iter-' + str(self.n_iter).zfill(2), bbox_inches='tight')
            plt.close('all')


class SNDetector(object):
    
    def __init__(self,n_consecutive_alerts=4,images_size=(4096,2048),flux_thres = 500.0,vel_flux_thres = 150.0,vel_satu = 3000.0):
        self.n_conditions = 7
    ...
    def pixel_discrimination(self,o,FH,KF):
        ...
        self.pixel_conditions[1,:] = KF.state[1,:] > self.vel_flux_thres*(self.vel_satu-np.minimum(KF.state[0,:],self.vel_satu))/self.vel_satu
    ...
    def filter_groups(self,FH,KF):
        ...
        for i in range(n_pixel_groups):
            ...
            brightPixels = np.logical_or(FH.flux[a:b,c:d] > 2*FH.flux[posY,posX],FH.science[a:b,c:d] > 2*FH.science[posY,posX])
            ...
            self.PGData['group_flags_map'][self.PGData['pixel_coords'][i][:,0],self.PGData['pixel_coords'][i][:,1]] = self.PGData['group_flags'][i]
            ...
            for c in range(len(self.CandData)+1):
                if c == len(self.CandData):
                    ...
                    if (np.sqrt(np.sum((cand_mid_coords[i,:] - self.CandData[c]['coords'])**2)) < 4.0):
                        ...
                        self.CandData[c]['coords'] = (self.CandData[c]['coords']*n_epochs + cand_mid_coords[i,:]) / (n_epochs+1)
...
class Observer(object):
    ...
    def new_objects_from_CandData(self,CandData):
        for i in range(len(CandData)):
            self.new_object(CandData[i]['coords'][0],CandData[i]['coords'][1],epochs=CandData[i]['epochs'],status=CandData[i]['status'])
    ...
    def print_lightcurve(self,MJD,obj,posY=-1,posX=-1,save_filename = '',SN_found=False):
        ...
        plt.errorbar(MJD+0.015,obj['state'][:,0,posY,posX],yerr=obj['state_cov'][:,0,posY,posX],fmt='b.-', label='Estimated flux')
        plt.errorbar(MJD-0.015,obj['pred_state'][:,0,posY,posX],yerr=obj['pred_state_cov'][:,0,posY,posX],fmt='g.', label='Predicted flux')
        plt.errorbar(MJD,obj['obs_flux'][:,posY,posX],yerr=obj['obs_var_flux'][:,posY,posX],fmt='r.', label='Observed flux')
        plt.grid()
        ...
        plt.errorbar(MJD,obj['state'][:,1,posY,posX],yerr=obj['state_cov'][:,2,posY,posX],fmt='b.-', label='Estimated flux velocity')
        plt.errorbar(MJD-0.03,obj['pred_state'][:,1,posY,posX],yerr=obj['pred_state_cov'][:,2,posY,posX],fmt='g.', label='Predicted flux velocity')
        plt.grid()
        plt.legend(loc=0,fontsize='small')
        plt.xlim(MJD[0]-1,MJD[-1]+1)
    ...
    def print_stamps(self,MJD,obj,save_filename = '',SN_found=False):
        ...
        plt.axis('off')
        plt.title('Science image, position: ' + str(obj['posY']) + ',' + str(obj['posX']) +', status: ' + str(obj['status']))
        ...
        plt.imshow(self.stack_stamps(obj['state'][:,0,:],MJD),vmin=-200,vmax=3000,cmap='Greys_r',interpolation='none')
        plt.title('Estimated flux')
        plt.axis('off')
        
        plt.subplot2grid((num_graphs,1),(5,0))
        plt.imshow(self.stack_stamps(obj['state'][:,1,:],MJD),vmin=-500,vmax=500,cmap='Greys_r',interpolation='none')
    ...
    def print_space_states(self,MJD,obj,posY=-1,posX=-1,save_filename = '',SN_found=False):
        ...
        plt.plot(obj['obs_flux'][:,posY,posX],np.diff(np.concatenate((np.zeros(1),obj['obs_flux'][:,posY,posX]))),'r.-',label='Observation',alpha=0.25)
        plt.grid()
    ...
    def print_all_space_states(self,fig,MJD,obj,sn,NUO,SN_found,save_filename = ''):
        ...
        plt.figure(fig)
        plt.plot(obj['state'][:,0,posY,posX],obj['state'][:,1,posY,posX],plot_color,label='Estimation',alpha=0.2)
