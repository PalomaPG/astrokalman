...
class KalmanFilter(object):
    ...
    def __init__(self, init_time=0.0, init_state=0.0, images_size=(4094, 2046), initial_variance=100.0, sigma=1000.0,
        ...
        self.num_cov_elements = int(self.num_states * (self.num_states + 1) / 2)
        ...
        self.state_cov = np.zeros(tuple([self.num_cov_elements]) + images_size, dtype=float)
    ...
    def predict_at_new_time(self, new_time):
        ...
        print('Orig program delta_t %f' % delta_t)
        #print('siffffff')
        #print(delta_t)
        ...
        reserve = self.state_cov[1, :] + delta_t * self.state_cov[2, :] # Parte de la ecuacion
        self.pred_state_cov[0, :] = self.state_cov[0, :] + delta_t * (self.state_cov[1, :] + reserve) + Q[0]
        self.pred_state_cov[1, :] = reserve + Q[1]
        self.pred_state_cov[2, :] = self.state_cov[2, :] + Q[2]
